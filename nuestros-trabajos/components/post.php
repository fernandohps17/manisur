          ...
          <div class='post'>
            <div class='img-portada'>
              <picture>
                ...
                <img itemprop='image' src='/public/img/blog/$page/$img_portada' title='" . html_entity_decode($title) . "' alt='" . html_entity_decode($title) . "'  width='100' height='100' />
              </picture>";
...
if ($login === true) {
  echo "
                <div class='btn-admin btn-edit-post'>
                  <form action='/nuestros-trabajos/editar-post/' method='POST'>
                ...
                <div class='btn-admin btn-delete-post'>
                  <form action='/nuestros-trabajos/borrar-post/' method='POST'>
                    <input type='hidden' name='id' value='$id'>
                    <button type='submit'><i class=''>
...
echo     "</div>
            <div class='title text-start'>
              <h1 itemprop='name'>" . html_entity_decode($title) . "</h1>
            </div>
            <div class='text breakpoint'>
              <a href='#comentario' title='Deja un Comentario'>Deja un comentario</a>
              <p> / </p>
              <a href='/nuestros-trabajos/' title='Trabajos'>Trabajos</a>
              <p> / </p>
              <a href='/' title='Manisur'>Por Manisur</a>
            </div>
...
echo    "</div>
            <div class='form-comentary' id='comentario'>
              ...
              <form action='/nuestros-trabajos/agregar-comentario/' method='POST'>
                    ...
                    <div class='comentary'>
                      <div class='author'>
                        ...
                        <div class='option-comentary'>
                          <form class='public-comentary' action='/nuestros-trabajos/active-comentario/' method='POST'>
                          ...
                          <form class='delete-comentary' action='/nuestros-trabajos/borrar-comentario/' method='POST'>
                            <input type='hidden' name='id' value='" . $value['id'] . "'>
                            <button type='submit'><i class=''>
                            <svg xmlns='http://www.w3.org/2000/svg' fill='#ffff' height='24' viewBox='0 -960 960 960' width='24'><path d='M280-120q-33 0-56.5-23.5T200-200v-520h-40v-80h200v-40h240v40h200v80h-40v520q0 33-23.5 56.5T680-120H280Zm400-600H280v520h400v-520ZM360-280h80v-360h-80v360Zm160 0h80v-360h-80v360ZM280-720v520-520Z'/></svg>
                            </i></button>
...
if (!empty($last_ten)) {
  foreach ($last_ten as $value) {
    if ($value['seo'] === "true") {
      ...
      $path_last_ten = "/nuestros-trabajos/post/" . $value['uri'] . "/";
    }

    echo "
              <div class='item-post'>
                <a href='$path_last_ten'>" . html_entity_decode($value['title']) . "</a>
                <p>" . $value['fecha'] . "</p>
              </div>
              ";
  }
} else {
  echo "<p>No hay más entradas</p>";
}
