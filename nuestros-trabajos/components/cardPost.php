...
function card_post($title, $img_mobile, $img_mobile_web, $description, $uri, $fecha, $id, $seo, $login)
{

  if ($seo === 'true') {
    $path = "/$uri/";
  } else {
    $path = "/nuestros-trabajos/post/$uri/";
  }
  ...
  echo    "<a href='" . $path . "' title='" . html_entity_decode($title) . "' class='img-post'>
                <picture>
                  <source type='image/webp' srcset='/public/img/blog/$uri/$img_mobile_web' media='(max-width: 700px)'>
                  <img itemprop='image' src='/public/img/blog/$uri/$img_mobile' alt='" . html_entity_decode($title) . "'  width='100' height='100'>
                </picture>
              </a>
              <div class='content-text'>
                <div class='title-post'>
                  <h2 itemprop='name'>" . html_entity_decode($title) . "</h2>
                </div>
                <div class='description-post'>
                  <p itemprop='articleBody'>" . html_entity_decode($description) . "</p>
                </div>
                <div class='info-post'>
                  <a itemprop='url' href='" . $path . "' title='" . html_entity_decode($title) . "'>Saber más</a>
                </div>
              </div>
